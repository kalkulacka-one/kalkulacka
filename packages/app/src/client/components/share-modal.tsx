...
export function ShareModal({ isOpen, onClose, onShare, buildShareUrl, privacyHref = "/soukromi" }: ShareModalProps) {
  const t = useTranslations("koa.components.shareModal");
  const tCommon = useTranslations("koa.pages.common");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose} role="dialog" aria-modal="true">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 relative" onClick={(e) => e.stopPropagation()} role="document">
        <button type="button" onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 transition-colors" aria-label={tCommon("close")}>
