...
export function Logo({ title, text }: Logo) {
  const titleId = useId();

  return (
    <div className="ko:flex ko:items-center ko:gap-[1.78125rem]">
      <svg
      ...
      {text && <div className="ko:font-display ko:font-bold ko:uppercase ko:tracking-[10%] ko:text-[2.25rem] ko:line-height-[normal] ko:whitespace-nowrap">{title}</div>}
    </div>
  );
}
