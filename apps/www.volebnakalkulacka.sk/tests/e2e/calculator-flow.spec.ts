...
const CALCULATORS: CalculatorConfig[] = [
  {
    ...
    name: "Inventúra hlasovaní",
    path: "/inventura-2023-2025",
    expectedTitle: "Inventúra hlasovaní Národnej rady Slovenskej republiky 2023-2025",
  },
];
...
async function navigateToNavod(page: Page, calculator: CalculatorConfig) {
  const navodButton = page.locator('a[href*="/navod"], button:has-text("Návod"), button:has-text("Pokračovať")').first();
...
async function startQuestions(page: Page, calculator: CalculatorConfig) {
  const questionButton = page
    .locator('a[href*="/otazka"], button:has-text("Rozumiem, začať"), button:has-text("Začať odpovedať"), button:has-text("Začať"), button:has-text("Pokračovať"), button:has-text("Prvá otázka")')
    .first();
...
async function answerQuestions(page: Page, maxQuestions = 3) {
  for (let i = 0; i < maxQuestions; i++) {
    try {
      ...
      const answerOptions = page.locator('button[role="radio"], input[type="radio"]').or(page.getByRole("button", { name: /^(Súhlasím|Nesúhlasím|Áno|Nie)$/ }));
      ...
      // Answering moves to the next question on its own; a "Ďalšia" is only there to press when the question was already answered.
      const nextButton = page.locator('button:has-text("Ďalšia"), button:has-text("Ďalej"), button:has-text("Pokračovať"), a:has-text("Ďalšia")').first();
  ...
  test.describe(`Calculator Flow: ${calculator.name}`, () => {
    test("should complete the full calculator flow from homepage", async ({ page }) => {
      ...
      if (await startQuestions(page, calculator)) {
        ...
        expect(currentUrl).toMatch(new RegExp(`.*${calculator.path}/(otazka|rekapitulacia|vysledok)`));
      }
    });
