...
describe("comparisonFilterQuery", () => {
  ...
  it("writes the starred questions and a topic slug on the one parameter", () => {
    expect(comparisonFilterQuery("important")).toBe(`?${COMPARISON_FILTER_PARAM}=dolezite`);
    expect(comparisonFilterQuery({ topic: "zivotne-prostredie" })).toBe(`?${COMPARISON_FILTER_PARAM}=zivotne-prostredie`);
  });
...
describe("parseComparisonFilter", () => {
  ...
  it("reads the starred questions and leaves any other value to the page as a topic slug", () => {
    expect(parseComparisonFilter("dolezite")).toBe("important");
    expect(parseComparisonFilter("doprava")).toEqual({ topic: "doprava" });
  });

  it("round-trips", () => {
    for (const filter of [undefined, "important" as const, { topic: "skolstvo" }]) {
