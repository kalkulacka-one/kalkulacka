// TODO [TENANT-005]: Translate share modal strings to Slovak
  ...
  useEffect(() => {
    if (isOpen && !publicId) {
      ...
      shareSession(calculatorId)
        ...
        .catch(() => {
          setError("Nepodarilo sa vytvoriť odkaz na zdieľanie");
        })
        .finally(() => {
          setIsLoading(false);
        });
  ...
  const handleDownloadInstagramStory = async () => {
    ...
    if (navigator.share) {
      try {
        ...
        const file = new File([blob], "Moja Volebná kalkulačka.png", { type: "image/png" });

        if (navigator.canShare?.({ files: [file] })) {
          await navigator.share({
            files: [file],
            title: "Volebná kalkulačka",
            text: "Pozrite sa, ako mi vyšla Volebná kalkulačka",
          });
          return;
        }
      } catch (error) {
    ...
    link.download = "Moja Volebná kalkulačka.png";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose} role="dialog" aria-modal="true">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 relative" onClick={(e) => e.stopPropagation()} role="document">
        <button type="button" onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 transition-colors" aria-label="Zavrieť">
          <Icon icon={mdiClose} size="medium" decorative />
        </button>

        <h2 className="font-display text-2xl font-bold text-slate-800 mb-4">Zdieľať výsledok</h2>

        {isLoading && (
          <div className="flex items-center justify-center py-8">
            <div className="text-slate-500">Vytváranie odkazu...</div>
          </div>
        )}
        ...
        {shareUrl && !isLoading && !error && (
          <>
            <p className="text-slate-600 text-sm mb-4">Zdieľajte svoj výsledok so svojimi blízkymi:</p>
              ...
              {hasClipboardAccess && (
                <div className="shrink-0">
                  <Button onClick={handleCopy} variant="outline" color="neutral" size="small">
                    ...
                    {isCopied ? "Skopírované" : "Kopírovať"}
                  </Button>
                </div>
              )}
            </div>
            ...
            <p className="text-slate-500 text-xs mt-4">
              Zdieľaním výsledkov súhlasíte s našimi{" "}
