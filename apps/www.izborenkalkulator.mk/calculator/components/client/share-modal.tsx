  ...
  useEffect(() => {
    if (isOpen && !publicId) {
      ...
      shareSession(calculatorId)
        ...
        .catch(() => {
          setError("Не успеа да се креира линк за споделување");
        })
        .finally(() => {
          setIsLoading(false);
        });
  ...
  const handleDownloadInstagramStory = async () => {
    ...
    if (navigator.share) {
      try {
        ...
        const file = new File([blob], "Мој Изборен калкулатор.png", { type: "image/png" });

        if (navigator.canShare?.({ files: [file] })) {
          await navigator.share({
            files: [file],
            title: "Изборен калкулатор",
            text: "Погледнете како ми излезе Изборниот калкулатор",
          });
          return;
        }
      } catch (error) {
    ...
    link.download = "Мој Изборен калкулатор.png";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose} role="dialog" aria-modal="true">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 relative" onClick={(e) => e.stopPropagation()} role="document">
        <button type="button" onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 transition-colors" aria-label="Затвори">
          <Icon icon={mdiClose} size="medium" decorative />
        </button>

        <h2 className="font-display text-2xl font-bold text-slate-800 mb-4">Сподели резултат</h2>

        {isLoading && (
          <div className="flex items-center justify-center py-8">
            <div className="text-slate-500">Креирање линк...</div>
          </div>
        )}
        ...
        {shareUrl && !isLoading && !error && (
          <>
            <p className="text-slate-600 text-sm mb-4">Споделете го вашиот резултат со блиските:</p>
            ...
            <p className="text-slate-500 text-xs mt-4">
              Со споделување на резултатите се согласувате со нашата{" "}
