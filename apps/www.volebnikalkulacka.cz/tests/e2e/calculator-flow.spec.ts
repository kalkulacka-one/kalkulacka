import messages from "@kalkulacka-one/app/locales/cs.json";
...
const electionPrefix = `/${slugs.prefixes.election}/`;
const pages = {
...
function electionLinks(html: string): string[] {
  const links = new Set<string>();
  for (const match of html.matchAll(/href="([^"]+)"/g)) {
    const href = match[1]?.replace(/&amp;/g, "&");
    if (href?.startsWith(electionPrefix)) links.add(toPath(href));
  }
  return [...links];
}

/** Follows election links from the homepage; a path whose root redirects to an introduction page is a calculator. */
async function discoverCalculators(context: APIRequestContext): Promise<string[]> {
  ...
  while (visited.size < limits.crawlPages) {
    ...
    if (location) {
      ...
      else if (target.startsWith(electionPrefix)) queue.push({ path: target, depth: next.depth });
      continue;
    }
    if (!response.ok() || next.depth >= limits.crawlDepth) continue;

    for (const link of electionLinks(await response.text())) {
...
test.describe("Calculator smoke", () => {
  ...
  test("an unknown calculator path is a not-found page, not a crash", async ({ page }) => {
    const response = await page.goto(`${electionPrefix}neexistujici-kalkulacka`, { timeout: timeouts.navigation });
