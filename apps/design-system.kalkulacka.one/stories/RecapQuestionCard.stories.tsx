...
import { RecapQuestionCard } from "@repo/design-system/server";
...
const meta: Meta<typeof RecapQuestionCard> = {
  title: "Components/RecapQuestionCard",
  component: RecapQuestionCard,
  tags: ["autodocs"],
  argTypes: {
    questionCurrent: {
      control: "number",
...
type RecapQuestionCardStory = StoryObj<typeof meta>;

export const Default: RecapQuestionCardStory = {
  args: {
    questionCurrent: 1,
  ...
  render: (args) => {
    ...
    return <RecapQuestionCard {...args} key={questions[args.questionCurrent]?.id} question={question} />;
  },
};

export const Group: RecapQuestionCardStory = {
  args: {
    questionCurrent: 1,
    questionTotal: questions.length,
  },
    ...
    return (
      <>
        {questions.map((question, index) => (
          <RecapQuestionCard {...args} key={question.id} question={question} questionCurrent={index + 1} questionTotal={questions.length} />
        ))}
      </>
    );
  },
