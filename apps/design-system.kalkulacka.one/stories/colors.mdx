import { Meta, Unstyled } from "@storybook/addon-docs/blocks";

<Meta title="Colors" />

# Colors

Kalkulacka.1 design system colors are designed to be easily themeable.

There are two layers of CSS variables:

- [**color palette**](#color-palette): base colors defined per theme
- [**color tokens**](#color-tokens): semantic colors derived from the palette

## Color palette

The color palette defines three main colors:

- primary: `--ko-palette-primary`
- secondary: `--ko-palette-secondary`
- neutral: `--ko-palette-neutral`

<Unstyled>
  <div
    style={{
      display: "grid",
      gridTemplateColumns: "repeat(auto-fill, minmax(8rem, 1fr))",
      gap: "2rem",
      backgroundColor: "rgba(0, 0, 0, 0.05)",
      padding: "2rem",
    }}
  >
    {[
      { label: "primary", variable: "--ko-palette-primary" },
      { label: "secondary", variable: "--ko-palette-secondary" },
      { label: "neutral", variable: "--ko-palette-neutral" },
    ].map(({ label, variable }) => (
      <div
        key={variable}
        style={{
          display: "flex",
          flexDirection: "column",
          alignItems: "center",
          gap: "0.5rem",
        }}
      >
        <div
          style={{
            width: "8rem",
            height: "8rem",
            borderRadius: "50%",
            backgroundColor: `var(${variable})`,
          }}
        />
        <div style={{ textAlign: "center", fontSize: "0.875rem" }}>{label}</div>
        <code style={{ textAlign: "center", fontSize: "0.625rem" }}>{variable}</code>
      </div>
    ))}
  </div>
</Unstyled>

and additional direct colors:

- red: `--ko-palette-red`
- yellow: `--ko-palette-yellow`
- green: `--ko-palette-green`

<Unstyled>
  <div
    style={{
      display: "grid",
      gridTemplateColumns: "repeat(auto-fill, minmax(8rem, 1fr))",
      gap: "2rem",
      backgroundColor: "rgba(0, 0, 0, 0.05)",
      padding: "2rem",
    }}
  >
    {[
      { label: "red", variable: "--ko-palette-red" },
      { label: "yellow", variable: "--ko-palette-yellow" },
      { label: "green", variable: "--ko-palette-green" },
    ].map(({ label, variable }) => (
      <div
        key={variable}
        style={{
          display: "flex",
          flexDirection: "column",
          alignItems: "center",
          gap: "0.5rem",
        }}
      >
        <div
          style={{
            width: "8rem",
            height: "8rem",
            borderRadius: "50%",
            backgroundColor: `var(${variable})`,
          }}
        />
        <div style={{ textAlign: "center", fontSize: "0.875rem" }}>{label}</div>
        <code style={{ textAlign: "center", fontSize: "0.625rem" }}>{variable}</code>
      </div>
    ))}
  </div>
</Unstyled>

If the default mapping of the main colors to the individual parts of the logo doesn't match the desired logo composition, individual logo colors can be adjusted by defining another set of specific colors:

- check: `--ko-palette-logo-check` *(primary as default)*
- slash: `--ko-palette-logo-slash` *(neutral as default)*
- cross: `--ko-palette-logo-cross` *(secondary as default)*
- percent numerator: `--ko-palette-logo-percent-numerator` *(primary as default)*
- percent slash: `--ko-palette-logo-percent-slash` *(neutral as default)*
- percent denominator: `--ko-palette-logo-percent-denominator` *(secondary as default)*

<Unstyled>
  <div
    style={{
      display: "grid",
      gridTemplateColumns: "repeat(auto-fill, minmax(8rem, 1fr))",
      gap: "2rem",
      backgroundColor: "rgba(0, 0, 0, 0.05)",
      padding: "2rem",
    }}
  >
    {[
      { label: "check", variable: "--ko-palette-logo-check" },
      { label: "slash", variable: "--ko-palette-logo-slash" },
      { label: "cross", variable: "--ko-palette-logo-cross" },
      { label: "percent numerator", variable: "--ko-palette-logo-percent-numerator" },
      { label: "percent slash", variable: "--ko-palette-logo-percent-slash" },
      { label: "percent denominator", variable: "--ko-palette-logo-percent-denominator" },
    ].map(({ label, variable }) => (
      <div
        key={variable}
        style={{
          display: "flex",
          flexDirection: "column",
          alignItems: "center",
          gap: "0.5rem",
        }}
      >
        <div
          style={{
            width: "8rem",
            height: "8rem",
            borderWidth: "0.25rem",
            borderStyle: "dashed",
            borderRadius: "50%",
            borderColor: `var(${variable}, black)`,
            backgroundColor: `var(${variable})`,
          }}
        />
        <div style={{ textAlign: "center", fontSize: "0.875rem" }}>{label}</div>
        <code style={{ textAlign: "center", fontSize: "0.625rem" }}>{variable}</code>
      </div>
    ))}
  </div>
</Unstyled>

### Theming

Themes live in the design system package at `/src/themes/{ full instance URL }/{ variant }.css`, for example `/design-system/src/themes/www.kalkulacka.one/default.css` (or `ct24.css` for embeds).

The theme file defines just the CSS variables for the palette colors, therefore a minimal color theme might look like this:

```css
:root {
  --ko-palette-primary: blue;
  --ko-palette-secondary: orange;
  --ko-palette-neutral: gray;
  --ko-palette-red: red;
  --ko-palette-yellow: yellow;
  --ko-palette-green: green;
}
```

## Color tokens

Color tokens assign semantic meaning to the palette colors and define [custom colors in Tailwind CSS](https://tailwindcss.com/docs/colors#customizing-your-colors).

Thanks to the support for [`light-dark()`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/light-dark) function and [relative color syntax](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_colors/Relative_colors) (CSS Color Module Level 5) and [Oklch color space](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/oklch) (CSS Color Module Level 4), tokens can be dynamically derived from the few base palette colors.

However, tokens can always be overridden with a specific palette variable for more precise control, for example if the primary/secondary colors don't align well with the affirmative/negative meaning of the response buttons.

### Structure

Color tokens are defined on four main levels:

- element
- variant
- state
- light/dark mode

In addition, combinations (element on token) with appropriate color contrast are also defined.

### Reference

#### Main colors

<Unstyled>
  <div
    style={{
      display: "grid",
      gridAutoFlow: "rows",
    }}
  >
    {["light", "dark"].map((mode) => (
      <div
        key={mode}
        style={{
          display: "grid",
          gridTemplateColumns: "repeat(auto-fill, minmax(8rem, 1fr))",
          gap: "2rem",
          backgroundColor: mode === "dark" ? "rgba(0, 0, 0, 0.85)" : "rgba(0, 0, 0, 0.05)",
          color: mode === "dark" ? "white" : "black",
          padding: "2rem",
        }}
      >
        {[
          { label: "primary", variable: "--ko-color-primary" },
          { label: "secondary", variable: "--ko-color-secondary" },
          { label: "neutral", variable: "--ko-color-neutral" },
        ].map(({ label, variable }) => (
          <div
            key={`${mode}-${variable}`}
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              gap: "0.5rem",
              colorScheme: mode,
            }}
          >
            <div
              style={{
                width: "8rem",
                height: "8rem",
                borderRadius: "50%",
                backgroundColor: `var(${variable})`,
              }}
            />
            <div style={{ textAlign: "center", fontSize: "0.875rem" }}>
              {label}
            </div>
            <code style={{ textAlign: "center", fontSize: "0.625rem" }}>{variable}</code>
          </div>
        ))}
      </div>
    ))}
  </div>
</Unstyled>

#### Direct colors

<Unstyled>
  <div
    style={{
      display: "grid",
      gridAutoFlow: "rows",
    }}
  >
    {["light", "dark"].map((mode) => (
      <div
        key={mode}
        style={{
          display: "grid",
          gridTemplateColumns: "repeat(auto-fill, minmax(8rem, 1fr))",
          gap: "2rem",
          backgroundColor: mode === "dark" ? "rgba(0, 0, 0, 0.85)" : "rgba(0, 0, 0, 0.05)",
          color: mode === "dark" ? "white" : "black",
          padding: "2rem",
        }}
      >
        {[
          { label: "red", variable: "--ko-color-red" },
          { label: "yellow", variable: "--ko-color-yellow" },
          { label: "green", variable: "--ko-color-green" },
        ].map(({ label, variable }) => (
          <div
            key={`${mode}-${variable}`}
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              gap: "0.5rem",
              colorScheme: mode,
            }}
          >
            <div
              style={{
                width: "8rem",
                height: "8rem",
                borderRadius: "50%",
                backgroundColor: `var(${variable})`,
              }}
            />
            <div style={{ textAlign: "center", fontSize: "0.875rem" }}>
              {label}
            </div>
            <code style={{ textAlign: "center", fontSize: "0.625rem" }}>{variable}</code>
          </div>
        ))}
      </div>
    ))}
  </div>
</Unstyled>

Surfaces and text are derived from the neutral, primary and secondary palette colors, so every existing theme gets a complete light and dark set for free. A theme pins any of them with `--ko-palette-page`, `--ko-palette-surface`, `--ko-palette-text`, … (or the `-light` / `-dark` variants where a theme needs one).

The "Ano" and "Ne" answer buttons use the primary and secondary tokens directly — there is a single `soft` / `wash` / `wash-strong` set per color, not a separate agree/disagree vocabulary.

#### Surfaces and text

<Unstyled>
  <div
    style={{
      display: "grid",
      gridAutoFlow: "rows",
    }}
  >
    {["light", "dark"].map((mode) => (
      <div
        key={mode}
        style={{
          display: "grid",
          gridTemplateColumns: "repeat(auto-fill, minmax(8rem, 1fr))",
          gap: "2rem",
          backgroundColor: mode === "dark" ? "rgba(0, 0, 0, 0.85)" : "rgba(0, 0, 0, 0.05)",
          color: mode === "dark" ? "white" : "black",
          padding: "2rem",
        }}
      >
        {[
          { label: "page", variable: "--ko-color-page" },
          { label: "surface", variable: "--ko-color-surface" },
          { label: "surface sunken", variable: "--ko-color-surface-sunken" },
          { label: "text", variable: "--ko-color-text" },
          { label: "text muted", variable: "--ko-color-text-muted" },
          { label: "border", variable: "--ko-color-border" },
          { label: "focus", variable: "--ko-color-focus" },
        ].map(({ label, variable }) => (
          <div
            key={`${mode}-${variable}`}
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              gap: "0.5rem",
              colorScheme: mode,
            }}
          >
            <div
              style={{
                width: "8rem",
                height: "8rem",
                borderRadius: "50%",
                backgroundColor: `var(${variable})`,
                boxShadow: "inset 0 0 0 1px rgba(128, 128, 128, 0.35)",
              }}
            />
            <div style={{ textAlign: "center", fontSize: "0.875rem" }}>
              {label}
            </div>
            <code style={{ textAlign: "center", fontSize: "0.625rem" }}>{variable}</code>
          </div>
        ))}
      </div>
    ))}
  </div>
</Unstyled>

#### Primary and secondary washes

<Unstyled>
  <div
    style={{
      display: "grid",
      gridAutoFlow: "rows",
    }}
  >
    {["light", "dark"].map((mode) => (
      <div
        key={mode}
        style={{
          display: "grid",
          gridTemplateColumns: "repeat(auto-fill, minmax(8rem, 1fr))",
          gap: "2rem",
          backgroundColor: mode === "dark" ? "rgba(0, 0, 0, 0.85)" : "rgba(0, 0, 0, 0.05)",
          color: mode === "dark" ? "white" : "black",
          padding: "2rem",
        }}
      >
        {[
          { label: "primary", variable: "--ko-color-primary" },
          { label: "primary soft", variable: "--ko-color-primary-soft" },
          { label: "on bg primary", variable: "--ko-color-on-bg-primary" },
          { label: "secondary", variable: "--ko-color-secondary" },
          { label: "secondary soft", variable: "--ko-color-secondary-soft" },
          { label: "on bg secondary", variable: "--ko-color-on-bg-secondary" },
          { label: "neutral", variable: "--ko-color-neutral" },
          { label: "neutral soft", variable: "--ko-color-neutral-soft" },
        ].map(({ label, variable }) => (
          <div
            key={`${mode}-${variable}`}
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              gap: "0.5rem",
              colorScheme: mode,
            }}
          >
            <div
              style={{
                width: "8rem",
                height: "8rem",
                borderRadius: "50%",
                backgroundColor: `var(${variable})`,
                boxShadow: "inset 0 0 0 1px rgba(128, 128, 128, 0.35)",
              }}
            />
            <div style={{ textAlign: "center", fontSize: "0.875rem" }}>
              {label}
            </div>
            <code style={{ textAlign: "center", fontSize: "0.625rem" }}>{variable}</code>
          </div>
        ))}
      </div>
    ))}
  </div>
</Unstyled>

### Logo colors

<Unstyled>
  <div
    style={{
      display: "grid",
      gridAutoFlow: "rows",
    }}
  >
    {["light", "dark"].map((mode) => (
      <div
        key={mode}
        style={{
          display: "grid",
          gridTemplateColumns: "repeat(auto-fill, minmax(8rem, 1fr))",
          gap: "2rem",
          backgroundColor: mode === "dark" ? "rgba(0, 0, 0, 0.85)" : "rgba(0, 0, 0, 0.05)",
          color: mode === "dark" ? "white" : "black",
          padding: "2rem",
        }}
      >
        {[
          { label: "check", variable: "--ko-color-logo-check" },
          { label: "slash", variable: "--ko-color-logo-slash" },
          { label: "cross", variable: "--ko-color-logo-cross" },
          { label: "percent numerator", variable: "--ko-color-logo-percent-numerator" },
          { label: "percent slash", variable: "--ko-color-logo-percent-slash" },
          { label: "percent denominator", variable: "--ko-color-logo-percent-denominator" },
        ].map(({ label, variable }) => (
          <div
            key={`${mode}-${variable}`}
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              gap: "0.5rem",
              colorScheme: mode,
            }}
          >
            <div
              style={{
                width: "8rem",
                height: "8rem",
                borderRadius: "50%",
                backgroundColor: `var(${variable})`,
              }}
            />
            <div style={{ textAlign: "center", fontSize: "0.875rem" }}>
              {label}
            </div>
            <code style={{ textAlign: "center", fontSize: "0.625rem" }}>{variable}</code>
          </div>
        ))}
      </div>
    ))}
  </div>
</Unstyled>

## Shape, motion and scale

Beyond colors, the design system defines a few non-color tokens that components share. Themes override them through `--ko-shape-*` and `--ko-motion-*` variables.

- **radius**: `--ko-radius-card` (the signature square top-left corner: `0 30px 30px 30px`), `--ko-radius-control` (fluid 16–22px), `--ko-radius-chip`, `--ko-radius-pill` — used as `ko:rounded-card`, `ko:rounded-control`, …; a theme pins them with `--ko-shape-radius-card`, …
- **motion**: `--ko-duration-fast` / `-base` / `-slow` (120 / 150 / 250 ms, collapsing to 0.01 ms under `prefers-reduced-motion`) and `--ko-ease-spring` / `--ko-ease-exit`; a theme pins them with `--ko-motion-duration-*` and `--ko-motion-easing-*`
- **elevation**: `--ko-shadow-card`, `-card-next`, `-card-back`, `-card-lifted`, `-sticky`, `-surface`, all tinted with the neutral palette color — used as `ko:shadow-card`, …
- **fluid scale**: sizes that ramp linearly between a 375px and a 1600px viewport via `clamp()`, in the spacing namespace (`ko:h-fluid-nav`, `ko:p-fluid-gutter`, `ko:size-fluid-star`, `ko:pt-fluid-card-pad-top`) and the text namespace (`ko:text-fluid-question`, `ko:text-fluid-gist`, `ko:text-fluid-chip`, `ko:text-title`, `ko:text-display`)
- **mode**: `data-mode="light"` or `data-mode="dark"` on the root element forces a color scheme; without it the page follows the operating system.
