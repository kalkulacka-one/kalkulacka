  ...
  useEffect(() => {
    if (isOpen && !publicId) {
      ...
      shareSession(calculatorId)
        ...
        .catch(() => {
          setError("A megosztási link létrehozása nem sikerült.");
        })
        .finally(() => {
          setIsLoading(false);
        });
      ...
      try {
        ...
        if (navigator.canShare?.({ files: [file] })) {
          await navigator.share({
            files: [file],
            title: "Voksmonitor",
            text: "Nézd meg, hogyan sikerült a Voksmonitorom!",
          });
          return;
        }
      } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose} role="dialog" aria-modal="true">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-6 relative" onClick={(e) => e.stopPropagation()} role="document">
        ...
        <h2 className="font-display text-2xl font-bold text-slate-800 mb-4">Eredmény megosztása</h2>

        {isLoading && (
          <div className="flex items-center justify-center py-8">
            <div className="text-slate-500">Link létrehozása...</div>
          </div>
        )}
